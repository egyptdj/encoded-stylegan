...
def D_wgan_gp(G, D, opt, shape, reals, # pylint: disable=unused-argument
    ...
    latents = tf.random_normal(shape)
    fake_images_out = G.get_output_for(latents, None, is_training=True)
    real_scores_out = fp32(D.get_output_for(reals, None, is_training=True))
    fake_scores_out = fp32(D.get_output_for(fake_images_out, None, is_training=True))
    ...
    with tf.name_scope('GradientPenalty'):
        ...
        mixed_scores_out = fp32(D.get_output_for(mixed_images_out, None, is_training=True))
        ...
        with tf.name_scope('GPU{}'.format(gpu_idx)), tf.device('/gpu:{}'.format(gpu_idx)):
            ...
            with tf.name_scope('loss'):
                ...
                with tf.name_scope('z_domain_loss'):
                    latent_critic = tflib.Network("z_critic", func_name='stylegan.training.networks_stylegan.G_mapping', dlatent_size=1, mapping_fmaps=args.latent_critic_hidden, mapping_layers=args.latent_critic_layers, latent_size=18*512, normalize_latents=False)

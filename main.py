...
def encode(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.compat.v1.variable_scope('encoder', reuse=reuse):
        ...
        for res in range(resolution_log2, last_res, -1):
            x = block(x, res)
        output = block(x, last_res)
...
def main():
    ...
    encoded_latents = encode(images, last_res=base_option['last_res'])
    ...
    recovered_encoded_images = Gs.components.synthesis.get_output_for(encoded_latents, None, is_validation=True, randomize_noise=True)
    smile_encoded_images = Gs.components.synthesis.get_output_for(latent_encoded_smile, None, is_validation=True, randomize_noise=True)
    ...
    with tf.name_scope('summary'):
        _ = tf.compat.v1.summary.scalar('encoding_loss', encoding_loss, family='loss', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        # _ = tf.compat.v1.summary.scalar('perceptual_loss', perceptual_loss, family='loss', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.scalar('lpips_loss', lpips_loss, family='loss', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.scalar('l2_loss', l2_loss, family='loss', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.scalar('total_loss', total_loss, family='loss', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.scalar('psnr', psnr, family='metrics', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.scalar('ssim', ssim, family='metrics', collections=['SCALAR_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.image('target', tf.clip_by_value(tf.transpose(images, perm=[0,2,3,1]), 0.0, 1.0), max_outputs=1, family='images', collections=['IMAGE_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.image('encoded', tf.clip_by_value(tf.transpose(encoded_images, perm=[0,2,3,1]), 0.0, 1.0), max_outputs=1, family='images', collections=['IMAGE_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.image('recovered(withnoise)', tf.clip_by_value(tf.transpose(recovered_encoded_images, perm=[0,2,3,1]), 0.0, 1.0), max_outputs=1, family='images', collections=['IMAGE_SUMMARY', tf.GraphKeys.SUMMARIES])
        _ = tf.compat.v1.summary.image('recovered_smile', tf.clip_by_value(tf.transpose(smile_encoded_images, perm=[0,2,3,1]), 0.0, 1.0), max_outputs=1, family='images', collections=['IMAGE_SUMMARY', tf.GraphKeys.SUMMARIES])
        scalar_summary = tf.compat.v1.summary.merge(tf.get_collection('SCALAR_SUMMARY'))
        image_summary = tf.compat.v1.summary.merge(tf.get_collection('IMAGE_SUMMARY'))
    ...
    with tf.name_scope("test_encode"):
        ...
        test_encoded_latent = encode(tf.transpose(test_image_input, perm=[0,3,1,2]), last_res=base_option['last_res'], reuse=True)
        test_recovered_image = tf.transpose(Gs.components.synthesis.get_output_for(test_encoded_latent, None, is_validation=True, use_noise=True, randomize_noise=True), perm=[0,2,3,1])
    ...
    train_summary_writer = tf.compat.v1.summary.FileWriter(base_option['result_dir']+'/summary/train')

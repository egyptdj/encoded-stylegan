...
def main():
    ...
    encoded_latents = encode(tf.get_default_graph().get_tensor_by_name('D_1/'+'cond/'*(np.log2(base_option['disc_resolution'])-2)+'{}x{}/'.format(base_option['disc_resolution'])+'Conv1_down/LeakyReLU/IdentityN:0'), resolution=base_option['disc_resolution'], reuse=False)
    ...
    with tf.name_scope("test_encode"):
        ...
        test_encoded_latent = encode(tf.get_default_graph().get_tensor_by_name('test_encode/D/'+'cond/'*(np.log2(base_option['disc_resolution'])-2)+'{}x{}/'.format(base_option['disc_resolution'])+'Conv1_down/LeakyReLU/IdentityN:0'), resolution=base_option['disc_resolution'], reuse=True)

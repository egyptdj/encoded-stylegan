...
def D_wgan_gp(G, D, opt, latent_shape, reals, labels=None, # pylint: disable=unused-argument
    ...
    with tf.name_scope('GradientPenalty'):
        mixing_factors = tf.random_uniform([latent_shape[0], 1, 1, 1], 0.0, 1.0, dtype=fake_images_out[0].dtype)
        mixed_images_out = tflib.lerp(tf.cast(reals, fake_images_out[0].dtype), fake_images_out[0], mixing_factors)

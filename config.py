...
desc        = 'pae'                                        # Description string included in result subdir name.
random_seed = 1000                                          # Global random seed.
dataset     = EasyDict()                                    # Options for dataset.load_dataset().
train       = EasyDict(func='train.train_progressive_autoencoder')  # Options for main training func.
...
coeff_lambda = EasyDict(l2=1.0, l1=0.0, vgg=1.0, lpips=0.0, mssim=0.0, logcosh=0.0)

# Dataset (choose one).
desc += '-ffhq';            dataset = EasyDict(data_dir=config.data_dir, verbose=True, tfrecord_dir='ffhq'); train.mirror_augment = True
...
# desc += '-preset-v2-1gpu'; num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8, 512: 4, 1024: 2}; sched.G_lrate_dict = {1024: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
desc += '-preset-v2-1gpu'; num_gpus = 1; sched.minibatch_base = 2; sched.minibatch_dict = {4: 2, 8: 2, 16: 2, 32: 2, 64: 2, 128: 2, 256: 2, 512: 2, 1024: 2}; sched.G_lrate_dict = {1024: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000

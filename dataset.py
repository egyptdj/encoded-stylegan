...
class DatasetEncodedStyleGAN(object):
    ...
    def build(self, uint=False, nchw=False):
        ...
        test_im = np.stack([np.array(PIL.Image.open(self.testim_dir+"/"+image_path)) for image_path in self.image_list], axis=0)
        ...
        self.test_image = tf.Variable(initial_value=test_im, shape=[None,3,1024,1024], dtype=tf.float32, name='test_image')

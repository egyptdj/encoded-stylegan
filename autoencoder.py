...
def AE_basic(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def block(x, res): # res = 2..resolution_log2
        with tf.variable_scope('%dx%d_encode' % (2**res, 2**res)):
            if res >= 3: # 8x8 and up
    ...
    assert dlatents.dtype == tf.as_dtype(dtype)
    ...
    combo_in = tf.cast(dlatents, dtype)
    lod_in = tf.cast(tf.get_variable('lod', initializer=np.float32(0.0), trainable=False), dtype)
    images_out = None
    ...
    # Building blocks.
    def block(x, res): # res = 2..resolution_log2
        with tf.variable_scope('%dx%d_decode' % (2**res, 2**res)):
            if res == 2: # 4x4
